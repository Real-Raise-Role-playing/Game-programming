...
public class PhotonManager : Photon.MonoBehaviour
{
    ...
    void CreatePlayer()
    {
        ...
        if (redTeamCount <= blueTeamCount)
        {
            Player.GetComponent<PlayerState>().teamNum = 0;
        }
        else
        {
            Player.GetComponent<PlayerState>().teamNum = 1;
        }
        ...
        pv.RPC("SetConnectplayerObjList", PhotonTargets.AllBufferedViaServer, Player.GetComponent<PlayerState>().teamNum);
    }
    */

    /*
    [PunRPC]
    void SetConnectplayerObjList(int teamNum)
    {
        if (teamNum == 0)
        {
            redTeamCount++;
    ...
    void SetDisConnectplayerObjList(int veiwId)
    {
        foreach (GameObject playerObj in playerObjList)
        {
            if (playerObj.GetComponent<PhotonView>().viewID == veiwId)
            {
                ...
                //if (playerObj.GetComponent<PlayerState>().teamNum == 0)
                //{
                //    redTeamCount--;
                //}
                //else
